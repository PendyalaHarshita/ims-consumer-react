...
export default class AuthenticationService{
    static async login(dealer){
        try{
            const response= await axios.post('http://localhost:8085/ims/login',dealer);
            ...
            if(response.data == true){
                ...
                return false;//return false for unsuccesful login
            }
        }
        catch(error){
    ...
    static async register(dealer){
        try{
            const response = await axios.post('http://localhost:8085/ims/reg',dealer);
